...
- **Global scope**: `C:/FTS/MED-AI/DemoRepo/.kilo/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/FTS/MED-AI/DemoRepo/.kilo/notes/`)
...
2. Glob `C:/FTS/MED-AI/DemoRepo/.kilo/notes/*.md` (if directory exists) — global notes
...
Global (C:/FTS/MED-AI/DemoRepo/.kilo/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/FTS/MED-AI/DemoRepo/.kilo/notes/` — works in any directory
