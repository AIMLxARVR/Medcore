...
def create_enhanced_cancer_consultation_interface(language="English"):
    ...
    with st.sidebar:
        ...
        # Reset button
        if language == "Bengali":
            if st.button("🔄 নতুন পরামর্শ শুরু করুন", width='stretch'):
                consultation.reset_consultation()
                st.rerun()
        else:
            if st.button("🔄 Start New Consultation", width='stretch'):
                consultation.reset_consultation()
                st.rerun()
...
def display_consultation_start(consultation: EnhancedCancerConsultationSession, language: str, lang_code: str):
    ...
    if language == "Bengali":
        ...
        if st.button("🚀 স্মার্ট প্রশ্নোত্তর শুরু করুন", type="primary", width='stretch'):
            st.rerun()
    else:
        st.markdown("""
        <div style="background: white; padding: 30px; border-radius: 15px; margin: 20px 0; 
        ...
        if st.button("🚀 Start Smart Questionnaire", type="primary", width='stretch'):
            st.rerun()
