...
def render_enhanced_cancer_voice_vision_interface(language: str):
    ...
    if audio_file or image_file:
        if language == "Bengali":
            if st.button("🚀 উন্নত ক্যান্সার বিশ্লেষণ শুরু করুন", type="primary", width='stretch'):
                process_enhanced_cancer_multimodal_input(audio_file, image_file, language)
        else:
            if st.button("🚀 Start Enhanced Cancer Analysis", type="primary", width='stretch'):
                process_enhanced_cancer_multimodal_input(audio_file, image_file, language)
...
def render_quick_risk_assessment(language: str):
    ...
    if language == "Bengali":
        if st.button("🧮 বিস্তারিত ঝুঁকি বিশ্লেষণ করুন", type="primary", width='stretch', key="cancer_risk_calculate_btn"):
        ...
        if st.button("🧮 Calculate Detailed Risk Analysis", type="primary", width='stretch', key="cancer_risk_calculate_btn"):
            calculate_and_display_cancer_risk(age, gender, smoking, alcohol, family_history, 
                                            diet_quality, exercise, sun_exposure, language, True)
