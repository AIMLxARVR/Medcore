# cancer_streamlit_integration.py - Complete integration file for cancer domain in main Streamlit app
...
# Import the cancer-specific modules
from cancer_consultation_system import (
    create_cancer_consultation_interface,
    CancerConsultationSession
)
...
def render_cancer_domain_app():
    """Main function to render the cancer domain app"""
    
    # Custom CSS for cancer domain
    st.markdown("""
    <style>
    /* Cancer domain specific styling */
    .cancer-header {
    ...
    @keyframes urgentPulse {
        0%, 100% { box-shadow: 0 4px 15px rgba(255, 55, 66, 0.5); }
        50% { box-shadow: 0 8px 25px rgba(255, 55, 66, 0.8); }
    }
    ...
    .protective-row { background: #e8f5e9; }
    .neutral-row { background: #f5f5f5; }
    .mild-risk-row { background: #fff3e0; }
    .risk-row { background: #ffebee; }
    .high-risk-row { background: #ffebee; }
    </style>
    """, unsafe_allow_html=True)
    
    # Language selection
    if 'cancer_app_language' not in st.session_state:
        st.session_state.cancer_app_language = 'English'
    
    # Sidebar configuration
    with st.sidebar:
        st.markdown("""
        <div class="cancer-header" style="padding: 15px; margin-bottom: 15px;">
            <h3 style="margin: 0;">⚙️ Cancer AI Settings</h3>
        </div>
        """, unsafe_allow_html=True)
        ...
        selected_language = st.radio(
            "🌐 Language / ভাষা",
            language_options,
            index=0 if st.session_state.cancer_app_language == "English" else 1,
            key="cancer_domain_language_selector"
        )
        
        if selected_language != st.session_state.cancer_app_language:
            st.session_state.cancer_app_language = selected_language
            st.rerun()
        ...
        advanced_reasoning = st.checkbox(
            "🧠 Advanced Reasoning" if selected_language == "English" else "🧠 উন্নত যুক্তি",
            value=True,
            key="cancer_advanced_reasoning_toggle",
            help="Enable step-by-step reasoning analysis" if selected_language == "English" 
                 else "ধাপে ধাপে যুক্তি বিশ্লেষণ সক্রিয় করুন"
        )
        
        risk_visualization = st.checkbox(
            "📊 Risk Visualization" if selected_language == "English" else "📊 ঝুঁকি ভিজুয়ালাইজেশন",
            value=True,
            key="cancer_risk_visualization_toggle",
            help="Show interactive risk assessment charts" if selected_language == "English"
                 else "ইন্টারঅ্যাক্টিভ ঝুঁকি মূল্যায়ন চার্ট দেখান"
        )
        
        emergency_detection = st.checkbox(
            "🚨 Emergency Detection" if selected_language == "English" else "🚨 জরুরি অবস্থা সনাক্তকরণ",
            value=True,
            key="cancer_emergency_detection_toggle",
            help="Automatic emergency situation detection" if selected_language == "English"
                 else "স্বয়ংক্রিয় জরুরি অবস্থা সনাক্তকরণ"
        )
        ...
        if selected_language == "Bengali":
            ...
            <div style="background: #e8f5e8; padding: 15px; border-radius: 10px;">
                <h4>🎯 ক্যান্সার AI বিশেষত্ব</h4>
                <ul style="margin: 10px 0; padding-left: 20px;">
                    <li>উন্নত লক্ষণ বিশ্লেষণ</li>
                    <li>ঝুঁকি কারণ মূল্যায়ন</li>
                    <li>পার্থক্যমূলক রোগ নির্ণয়</li>
            ...
            info_text = """
            <div style="background: #e8f5e8; padding: 15px; border-radius: 10px;">
                <h4>🎯 Cancer AI Specialization</h4>
                <ul style="margin: 10px 0; padding-left: 20px;">
    ...
    if selected_language == "Bengali":
        ...
        <div class="cancer-header">
            <h1 style="margin: 0; font-size: 2.5em;">🎯 ক্যান্সার AI বিশেষজ্ঞ</h1>
            <p style="margin: 10px 0 0 0; font-size: 1.2em; opacity: 0.9;">
                উন্নত যুক্তি ও বিশ্লেষণ সহ বিশ্বমানের ক্যান্সার ঝুঁকি মূল্যায়ন
            </p>
        </div>
        """, unsafe_allow_html=True)
    else:
        st.markdown("""
        <div class="cancer-header">
            <h1 style="margin: 0; font-size: 2.5em;">🎯 AI Cancer Specialist</h1>
            <p style="margin: 10px 0 0 0; font-size: 1.2em; opacity: 0.9;">
                World-class cancer risk assessment with advanced reasoning and analysis
            </p>
        </div>
        """, unsafe_allow_html=True)
    ...
    if selected_language == "Bengali":
        ...
        <div class="feature-highlight">
            <h3 style="margin: 0 0 15px 0;">🌟 নতুন AI ক্ষমতা</h3>
            <div style="display: flex; justify-content: space-around; flex-wrap: wrap;">
                <div style="text-align: center; margin: 10px;">
                    <div style="font-size: 2em;">🧠</div>
                    <div>স্মার্ট যুক্তি</div>
                </div>
                <div style="text-align: center; margin: 10px;">
                    <div style="font-size: 2em;">📊</div>
                    <div>ঝুঁকি বিশ্লেষণ</div>
                </div>
                <div style="text-align: center; margin: 10px;">
                    <div style="font-size: 2em;">🚨</div>
                    <div>জরুরি সনাক্তকরণ</div>
                </div>
                <div style="text-align: center; margin: 10px;">
                    <div style="font-size: 2em;">🎯</div>
                    <div>ব্যক্তিগত পরামর্শ</div>
                </div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        st.markdown("""
        <div class="feature-highlight">
            <h3 style="margin: 0 0 15px 0;">🌟 New AI Capabilities</h3>
            <div style="display: flex; justify-content: space-around; flex-wrap: wrap;">
                <div style="text-align: center; margin: 10px;">
                    <div style="font-size: 2em;">🧠</div>
                    <div>Smart Reasoning</div>
                </div>
                <div style="text-align: center; margin: 10px;">
                    <div style="font-size: 2em;">📊</div>
    ...
    # Tab 1: Cancer Consultation
    with tab1:
        create_cancer_consultation_interface(selected_language)
    
    # Tab 2: Voice + Vision Cancer Analysis
    with tab2:
        render_cancer_voice_vision_interface(selected_language, advanced_reasoning)
    
    # Tab 3: Risk Calculator
    with tab3:
        render_cancer_risk_calculator(selected_language, risk_visualization)
    
    # Tab 4: AI Reasoning Viewer
    with tab4:
        render_reasoning_viewer(selected_language)

def render_reasoning_viewer(language: str):
    """Render AI reasoning process viewer"""
    
    if language == "Bengali":
        st.markdown("""
        <div class="reasoning-card">
            <h2 style="margin: 0;">🧠 AI যুক্তি প্রক্রিয়া ভিউয়ার</h2>
            <p style="margin: 10px 0 0 0;">AI কীভাবে সিদ্ধান্ত নেয় তা দেখুন</p>
        </div>
        """, unsafe_allow_html=True)
    else:
        st.markdown("""
        <div class="reasoning-card">
            ...
            <p style="margin: 10px 0 0 0;">See how AI makes decisions</p>
        </div>
        """, unsafe_allow_html=True)
    
    # Check if there's any reasoning data in session state
    reasoning_data = get_latest_reasoning_data()
    
    if reasoning_data:
        display_reasoning_trace(reasoning_data, language)
    else:
        if language == "Bengali":
        ...
        if language == "Bengali":
            if st.button("🎬 ডেমো যুক্তি প্রক্রিয়া দেখুন"):
                display_demo_reasoning_process(language)
        else:
            if st.button("🎬 Show Demo Reasoning Process"):
                display_demo_reasoning_process(language)

def display_reasoning_trace(reasoning_data: dict, language: str):
    """Display the reasoning trace in an interactive format"""
...
def get_latest_reasoning_data():
    """Get the latest reasoning data from session state"""
    
    # Check for reasoning data in different session keys
    reasoning_keys = [
        'cancer_consultation_en',
        'cancer_consultation_bn'
    ]
...
def display_demo_reasoning_process(language: str):
    """Display a demo reasoning process"""
    
    if language == "Bengali":
        demo_steps = [
            {
                "step": "লক্ষণ বিশ্লেষণ",
                "reasoning": "রোগী দীর্ঘস্থায়ী কাশি এবং ওজন হ্রাসের কথা বলেছেন। এই লক্ষণগুলি ফুসফুস ক্যান্সারের সাথে সামঞ্জস্যপূর্ণ হতে পারে।",
                "confidence": 0.75
            },
            {
                "step": "ঝুঁকি মূল্যায়ন", 
                "reasoning": "রোগীর ধূমপানের ইতিহাস এবং ৫০+ বয়স ফুসফুস ক্যান্সারের ঝুঁকি বাড়ায়।",
                "confidence": 0.85
            },
            {
                "step": "সুপারিশ প্রস্তুতি",
                "reasoning": "তাৎক্ষণিক চেস্ট এক্স-রে এবং পুলমোনোলজিস্টের পরামর্শ প্রয়োজন।",
                "confidence": 0.90
            }
        ]
    else:
        demo_steps = [
            {
                ...
                "reasoning": "Patient reports persistent cough and weight loss. These symptoms may be consistent with lung cancer.",
                "confidence": 0.75
            },
            {
                "step": "Risk Assessment",
                "reasoning": "Patient's smoking history and age 50+ increases lung cancer risk significantly.",
                "confidence": 0.85
            },
            {
                "step": "Recommendation Generation", 
                "reasoning": "Immediate chest X-ray and pulmonologist consultation are needed.",
                "confidence": 0.90
            }
        ]
...
def render_cancer_voice_vision_interface(language: str, advanced_reasoning: bool):
    """Enhanced voice and vision interface for cancer domain"""
    ...
    if audio_file or image_file:
        if language == "Bengali":
            if st.button("🚀 ক্যান্সার বিশ্লেষণ শুরু করুন", type="primary", use_container_width=True):
                process_cancer_multimodal_input(audio_file, image_file, language, advanced_reasoning)
        else:
            if st.button("🚀 Start Cancer Analysis", type="primary", use_container_width=True):
                process_cancer_multimodal_input(audio_file, image_file, language, advanced_reasoning)

def process_cancer_multimodal_input(audio_file, image_file, language: str, advanced_reasoning: bool):
    """Process voice and vision input for cancer analysis"""
    ...
    try:
        ...
        if image_file:
            with st.status("📷 Analyzing image..." if language == "English" else "📷 ছবি বিশ্লেষণ করা হচ্ছে..."):
                ...
                cancer_image_prompt = get_cancer_image_analysis_prompt(lang_code)
        ...
        if advanced_reasoning and combined_input:
        ...
        # Display results
        display_multimodal_cancer_results(transcribed_text, image_analysis, analysis_results, comprehensive_response, language, advanced_reasoning)
        
    except Exception as e:
        logging.error(f"Error in cancer multimodal processing: {e}")
        error_msg = (
...
def get_cancer_image_analysis_prompt(lang_code: str) -> str:
    """Get cancer-specific image analysis prompt"""
...
- ত্বকের কোনো পরিবর্তন বা নতুন দাগ
...
def display_multimodal_cancer_results(transcribed_text: str, image_analysis: str, analysis_results: dict, comprehensive_response: str, language: str, advanced_reasoning: bool):
    """Display results from multimodal cancer analysis"""
    ...
    if advanced_reasoning and analysis_results and comprehensive_response:
        # Show urgency level
        urgency_level = determine_urgency_from_analysis(analysis_results)
        display_urgency_alert(urgency_level, language)
...
def display_urgency_alert(urgency_level: str, language: str):
    """Display urgency alert based on analysis"""
...
def determine_urgency_from_analysis(analysis_results: dict) -> str:
...
def render_cancer_risk_calculator(language: str, risk_visualization: bool):
    """Render interactive cancer risk calculator with enhanced analysis"""
    ...
    if language == "Bengali":
        ...
        st.markdown("""
        <div class="reasoning-card">
            <h2 style="margin: 0;">📊 Cancer Risk Calculator</h2>
    ...
    if language == "Bengali":
        if st.button("🧮 বিস্তারিত ঝুঁকি বিশ্লেষণ করুন", type="primary", use_container_width=True, key="cancer_risk_calculate_btn"):
            calculate_and_display_cancer_risk(age, gender, smoking, alcohol, family_history, 
                                            diet_quality, exercise, sun_exposure, language, risk_visualization)
    else:
...
def display_detailed_risk_breakdown(risk_assessment: dict, reasoning_engine, language: str):
    ...
    if cancer_risks:
        ...
        for cancer_type, risk_data in cancer_risks.items():
            ...
            with st.expander(f"{cancer_name} - {risk_level.title() if language == 'English' else risk_level}"):
                st.markdown(f"""
                <div style="border-left: 4px solid {color}; padding: 15px; margin: 10px 0;">
                    <h4 style="color: {color}; margin: 0 0 10px 0;">{cancer_name}</h4>
                    <div style="background: #e0e0e0; border-radius: 10px; height: 15px; margin: 10px 0;">
                        <div style="background: {color}; height: 15px; border-radius: 10px; width: {risk_score*100}%;"></div>
                    </div>
                    <p><strong>{'ঝুঁকি স্কোর' if language == 'Bengali' else 'Risk Score'}:</strong> {risk_score:.1%}</p>
                    <p><strong>{'অবদানকারী কারণ' if language == 'Bengali' else 'Contributing Factors'}:</strong></p>
    ...
    # Show screening recommendations
    screening_recommendations = risk_assessment.get("recommended_screenings", [])
    if screening_recommendations:
        ...
        for recommendation in screening_recommendations:
