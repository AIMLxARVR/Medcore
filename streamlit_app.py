...
def main():
    ...
    with st.sidebar:
        ...
        # Control buttons
        if selected_language == "Bengali":
            if st.button("🗑️ চ্যাট রিসেট করুন", type="secondary", width='stretch'):
                reset_enhanced_chat_session(lang_code)
                st.success("চ্যাট রিসেট সম্পন্ন!")
                time.sleep(1)
                st.rerun()
        else:
            if st.button("🗑️ Reset Chat", type="secondary", width='stretch'):
                reset_enhanced_chat_session(lang_code)
        ...
        if chat_session.history:
            if selected_language == "Bengali":
                if st.button("📥 পরামর্শ ডাউনলোড করুন", width='stretch'):
                ...
                if st.button("📥 Export Consultation", width='stretch'):
                    json_data, filename = export_consultation_history(chat_session, lang_code)
                    st.download_button(
                        label="💾 Download File",
                        data=json_data,
